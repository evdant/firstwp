...
class WPCustomerReviews {
    ...
    function get_options() {
        ...
        $default_options = array(
            ...
            'field_custom' => array(),
            'form_location' => 0,
            'goto_leave_text' => 'Оставить отзыв.',
            'goto_show_button' => 1,
            'hreview_type' => 'business',
            'leave_text' => 'Оставьте ваш отзыв',
            ...
            'show_hcard' => 1,
            'show_hcard_on' => 1,
            'submit_button_text' => 'Оставьте ваш отзыв',
            'support_us' => 0,
            'title_tag' => 'h2'
        );
        ...
        if (count($reviews) == 0) {
            ...
            foreach ($reviews as $review) {
                ...
                $name_block = '' .
                    ...
                    '<span class="' . $hide_name . '"> Добавил </span>&nbsp;' .
    ...
    function show_reviews_form() {
        ...
        if ($this->options['ask_fields']['fwebsite'] == 1) {
            ...
            $fields .= '<tr><td><label for="' . $rand_prefixes[2] . '-fwebsite" class="comment-field">Сайт: ' . $req . '</label></td><td><input class="text-input" type="text" id="' . $rand_prefixes[2] . '-fwebsite" name="' . $rand_prefixes[2] . '-fwebsite" value="' . $this->p->fwebsite . '" /></td></tr>';
        }
        if ($this->options['ask_fields']['ftitle'] == 1) {
            ...
            $fields .= '<tr><td><label for="' . $rand_prefixes[3] . '-ftitle" class="comment-field">Тема отзыва: ' . $req . '</label></td><td><input class="text-input" type="text" id="' . $rand_prefixes[3] . '-ftitle" name="' . $rand_prefixes[3] . '-ftitle" maxlength="150" value="' . $this->p->ftitle . '" /></td></tr>';
        }
        ...
        foreach ($this->options['require_fields'] as $col => $val) {
            if ($val == 1) {
                ...
                $some_required = '<small>Обязательные поля отмечены *</small>';
            }
        }
                    ...
                    <form class="wpcrcform" id="wpcr_commentform" method="post" action="javascript:void(0);">
                        <div id="wpcr_div_2">
                            ...
                            <table id="wpcr_table_2">
                                <tbody>
                                    <tr><td colspan="2"><div id="wpcr_postcomment"> Написать отзыв </div></td></tr>
                                    ' . $fields;

        $out2 = '   
            <tr>
                <td><label class="comment-field">Оценка:</label></td>
                <td><div class="wpcr_rating">' . $this->output_rating(0, true) . '</div></td>
            </tr>';

        $out3 = '
                            <tr><td colspan="2"><label for="' . $rand_prefixes[5] . '-ftext" class="comment-field">Ваш отзыв:</label></td></tr>
                            ...
                            <tr>
                                <td colspan="2" id="wpcr_check_confirm">
                                    ...
                                    <div class="wpcr_fl"><input type="checkbox" name="' . $rand_prefixes[7] . '-fconfirm2" id="fconfirm2" value="1" /></div><div class="wpcr_fl" style="margin:-2px 0px 0px 5px"><label for="fconfirm2">Подтвердите, что вы не спамер</label></div>
                                    <div class="wpcr_clear"></div>
    ...
    function add_review($pageID) {
        ...
        return array(false, '<div>Спасибо за ваш отзыв. После проверки администратором сайта он будет опубликован.</div>');
    }
